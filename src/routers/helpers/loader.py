...
def load_scenario_to_dynamodb(scenario_file_path):
    ...
    print(
        f"シナリオ '{scenario_data.get('name', 'Unknown')}' (ID: {scenario_id}) を読み込み中..."
    )
    ...
    try:
        ...
        main_item = {
            ...
            "current_month": convert_to_dynamodb_format(
                scenario_data.get("current_month", 0)
            ),
...
def delete_scenario_from_dynamodb(scenario_id):
    ...
    try:
        # シナリオアイテムを削除
        table.delete_item(Key={"PK": "scenario", "SK": scenario_id})
...
def list_costs_in_dynamodb():
    ...
    try:
        # コストデータを取得
        response = table.get_item(Key={"PK": "costs", "SK": "metadata"})
...
def delete_costs_from_dynamodb() -> bool:
    ...
    try:
        # コストデータを削除
        table.delete_item(Key={"PK": "costs", "SK": "metadata"})
...
def main():
    ...
    parser.add_argument(
        "--load-costs", type=str, help="読み込むコストJSONファイルのパス"
    )
    parser.add_argument(
        "--list", action="store_true", help="DynamoDB内のシナリオ一覧を表示"
    )
    parser.add_argument(
        "--list-costs", action="store_true", help="DynamoDB内のコストデータを表示"
    )
    parser.add_argument("--delete", type=str, help="削除するシナリオID")
    parser.add_argument(
        "--delete-costs", action="store_true", help="コストデータを削除"
    )
    ...
    if args.load:
        ...
        print("使用方法:")
        print(
            "  シナリオを読み込む: python loader.py --load scenarios/personal_blog_scenario.json"
        )
        print(
            "  コストを読み込む: python loader.py --load-costs costs/dynamodb_costs.json"
        )
