...
class ScenarioService:
    ...
    async def get_all_scenarios(self) -> List[ScenarioSummary]:
        """全シナリオの一覧を取得"""
        try:
            response = self.table.query(KeyConditionExpression=Key("PK").eq("scenario"))
    ...
    async def get_scenario_by_id(
        self, scenario_id: str, include_requests: bool = True
    ) -> Scenario:
        """指定されたシナリオの詳細を取得"""
        try:
            # メインシナリオデータを取得
            response = self.table.get_item(Key={"PK": "scenario", "SK": scenario_id})
    ...
    async def get_month_data(self, scenario_id: str, month: int) -> MonthData:
        ...
        try:
            ...
            if not month_request:
                raise HTTPException(
                    status_code=404, detail=f"月 {month} のデータが見つかりません"
                )
    ...
    async def get_feature_by_id(self, feature_id: str) -> FeatureDetail:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"フィーチャー取得エラー: {str(e)}"
            )

    async def calculate_scenario_cost(
        self, scenario_id: str, month: int
    ) -> CostCalculationResult:
        """指定された月のシナリオコストを計算"""
        try:
            # 月データを取得
            ...
            costs_response = self.table.query(
                KeyConditionExpression=Key("PK").eq("costs")
                & Key("SK").begins_with("metadata")
            )
            costs_db = costs_response.get("Items", [{}])[0].get("costs", {})

            if not costs_db:
                raise HTTPException(
                    status_code=404, detail="コストデータが見つかりません"
                )
