...
from routers.helpers.cost_calculator import (
...
@play_router.post("/play/calculate-cost")
async def calculate_cost_only(
    request: dict, user_id: str = Depends(extract_user_id_from_token)
):
    """
    構造データのコスト計算のみを実行する（他のエンドポイントからの利用例）
    """
    try:
        struct_data = request.get("struct_data", {})
        month_requests = request.get("month_requests", 0)

        # コスト計算を実行
        total_cost, resource_costs = await calculate_infrastructure_cost(
            struct_data, month_requests
        )
...
async def report_game(game_id: str, user_id: str = Depends(extract_user_id_from_token)):
    ...
    try:
        ...
        game_over = check_game_over(total_cost, scenario_funds)
